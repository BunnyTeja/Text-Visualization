...
# Convering training data to tokenized form for heirarchical transformer
...
print("\n ---- PERFORMANCE METRICS FOR HEIRARCHICAL TRANSFORMER ----\n")
get_performance(X_test_final, Y_test)
...
count_vect, tfidf_transformer, clf = mnb(X_train, Y_train)
...
# Testing the old KITE model
Y_pred_nb = []
for doc in X_test:
    a = classify(doc)
    Y_pred_nb.append(np.argmax(a[0]))

# Performance metrics of old KITE modeL
print("\n ---- PERFORMANCE METRICS FOR CURRENT KITE MODEL ----\n")
...
fname = check_unseen(urls, 'v7')

# Printing the compiled results in tabular format
