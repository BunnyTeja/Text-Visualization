...
def jel_load_data(path):

    with open(path+'/vis/Data//jel_code_to_term.pk', 'rb') as f:
        code_to_term = pk.load(f)
    with open(path+'/vis/Data/jel_term_to_code.pk', 'rb') as g:
...
def get_codes_class_jel(text, term_to_code, code_to_term, triee, to_print = 0):
